...
namespace SpiderDocsWebAPIs
{
    ...
    public class SpiderDoscWebClient: ISDWebClient
    {
        ...
        public int EditAttributeComboboxItem(int AttributeId, DocumentAttributeCombo Item)
        {
            ...
            request.AddJsonBody(new { AttributeId = AttributeId, Item  = Item });
        ...
        public string[] GetDownloadUrls(int[] versionIds)
        {
            ...
            request.RequestFormat = DataFormat.Json;
            request.AddJsonBody(new { VersionIds = versionIds });
            LastResponse = Execute(request);
        ...
        // public bool RemoteImport(string url, Document doc, List<DocumentAttributeCombo> comboItem)
        // {
        ...
        public bool SaveDoc(string filepath, Document doc, out Document result)
        {
            ...
            request = new RestRequest(requestPath("SaveDoc"), Method.POST);
            request.RequestFormat = DataFormat.Json;
            request.AddParameter("Application/Json", ConvertToJson(new { TempId = tempID, Document = doc, Property = true }), ParameterType.RequestBody);
            response = Execute(request);
        ...
        public bool CheckOut(int[] DocIds/*, int[] FolderIds*/)
        {
            ...
            request.RequestFormat = DataFormat.Json;
            request.AddJsonBody(new { DocIds = DocIds/*, FolderIds = FolderIds */});
            LastResponse = Execute(request);
        ...
        public bool CancelCheckOut(int[] DocIds)
        {
            ...
            request.RequestFormat = DataFormat.Json;
            request.AddJsonBody(new { DocIds = DocIds });
            LastResponse = Execute(request);
        ...
        public History[] GetHistories(SearchCriteria Criteria)
        {
            ...
            request.RequestFormat = DataFormat.Json;
            request.AddJsonBody(new { Criteria  = Criteria });
            LastResponse = Execute(request);
        ...
        public bool Delete(int[] DocumentIds, string Reason)
        {
            ...
            request.RequestFormat = DataFormat.Json;
            request.AddJsonBody(new { DocumentIds = DocumentIds, Reason= Reason });
            LastResponse = Execute(request);
        ...
        public List<DocumentAttributeCombo> GetAttributeComboboxItems(int idAttr = 0, int idItem = 0)
        {
            ...
            //request.AddJsonBody(new { AttributeId = idAttr, ItemId = idItem });
            LastResponse = Execute(request);
		...
		string ConvertToJson(object value)
		{
			var settings = new JsonSerializerSettings
 			...
 			var json = JsonConvert.SerializeObject(value,Newtonsoft.Json.Formatting.None, settings);
